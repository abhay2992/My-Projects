...
class form9 extends JFrame implements ActionListener
{
	...
	txt8,txt9,txt10,txt11,txt12,txt13,txt14;
		...
		form9()
	{
		...
		getContentPane().add(jp);
			lb=new JLabel("RETRIEVE TEACHER DETAIL");
...
public void actionPerformed(ActionEvent e)
			...
			try
			{
				...
				ps=con.prepareStatement("select * from teacherdetail where id=?");
				ps.setString(1,code);
				ResultSet rs;
				rs=ps.executeQuery();
	...
	msg.showMessageDialog(null,"VALID ID","Information",1);
		...
		{
			msg.showMessageDialog(null," Record is not Found","Information",1);
		}
	...
	try
	{
		Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
		con=DriverManager.getConnection("Jdbc:Odbc:data");
		ps=con.prepareStatement("insert into upteacher values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	catch(Exception ee)
{
	msg.showMessageDialog(null,"RECORD IS ALREADY EXIST"+ee,"Information",1);
		...
		Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
		con=DriverManager.getConnection("Jdbc:Odbc:data");
		ps=con.prepareStatement("delete * from teacherdetail where id=?");
		ps.setString(1,txt1.getText());
		ps.executeUpdate();
